...
class CSIDataset(Dataset):
    ...
    def __init__(self, csv_files, window_size=32, step=1,transforms=None):
        ...
        for i in range(4):
            self.phases_pca.append(
                pca.fit_transform(self.phases[:, i * SUBCARRIES_NUM_FIVE_HHZ:(i + 1) * SUBCARRIES_NUM_FIVE_HHZ]))
        self.phases_pca = np.array(self.phases_pca)
        self.phases_pca = self.phases_pca.reshape((self.phases_pca.shape[1], self.phases_pca.shape[0] * self.phases_pca.shape[2]))
    ...
    def __getitem__(self, idx):
        ...
        for index in range(idx, idx + self.window):
            all_xs.append(np.append(np.append(np.append(self.amplitudes[index], self.amplitudes_pca[index]),self.phases[index]),self.phases_pca[index])) #,np.append( self.phases[index],self.phases_pca[index]))
            #all_ys.append(np.append( self.phases[index],self.phases_pca[index]))
            #all.append(np.append(all_xs,all_ys))

            # all_ys.append(self.class_to_idx[self.labels[index]])
            #all_xs.append(self.amplitudes[index])
            #all_xs.append( self.amplitudes_pca[index])
        if self.transforms:
            all_xs=self.transforms(all_xs)
...
if __name__ == '__main__':
    ...
    first_data = val_dataset[0]
    features, l= first_data
